 ...
 */
export function getRandomBetween(start, end) {
 ...
 * @param {number} p - 可能性
 */
export function possibility(p) {
    return Math.random() < p;
}
/** 车牌号可用的字母 */
export const plateNoAlphabetList = ["A", "B", "C", "D", "E", "F", "G", "H", "J", "K", "L", "M", "N", "P", "Q", "R", "S", "T", "U", "V", "W", "X", "Y", "Z"];
/**
 * 获取随机一位车牌号
 */
 ...
 */
export function getRandomPlateNo(start) {
 ...
 *
 */
export function getRandomId(area) {
...
/** 判断两个数字是否相等 */
export function twoNumberIsEqual(a, b) {
    if (isNaN(a) && isNaN(b))
        return true;
    if (a === b)
        return true;
 ...
 */
export function getCoord(coord1, coord2, d1, d2) {
    const [lat1, lng1] = coord1;
    const [lat2, lng2] = coord2;
 ...
 */
export function getProperties(obj, ...keyList) {
    const result = {};
    keyList.forEach(key => {
        result[key] = obj[key];
    });
 ...
 */
export function compareWithoutProperties(a, b, ...ignoreList) {
 ...
 */
export function compareProperties(a, b, ...keyList) {
    if (keyList.length === 0)
 ...
 */
export function stringToArray(id) {
 ...
 * @param {string} id - 身份证号
 */
export function coverIdWithMosaics(id) {
    if (isLegalId(id)) {
 ...
 */
export function stringToNumber(value, option) {
 ...
 * @param {string} id - 身份证号
 */
export function getAgeFromId(id) {
    if (!isLegalId(id))
        throw new Error("非法身份证号");
 ...
 * @param {string} id - 身份证号
 */
export function getSexFromId(id) {
    if (!isLegalId(id))
        throw new Error("非法身份证号");
 ...
 */
export function setPeriod(callback, period) {
    callback();
 ...
 */
export function addZero(number, length = 2) {
...
export function getPropertiesIsModified(a, b) {
...
/** 获得一个函数循环出来的数组 */
export function getArray(length, fun) {
    return Array(length)
        .fill(0)
        .map(($, index) => fun(index));
}
/** 获取点到线的最短距离 */
export function getPointToLineMinDistance(point, line, getDis) {
...
export function coordStringToNumber(str) {
    if (str.includes(":")) {
...
export function coordIsNumberArray(coord) {
    if (coord.length !== 2)
...
/** 检查坐标是否合法，合法则返回正确的坐标 */
export function coordCheck(coord) {
    if (coord.length !== 2)
        throw new Error(`${JSON.stringify(coord)} 的长度不为2`);
...
export function getRealCoord(coord) {
...
/** 将任意格式的坐标转换为51坐标 */
export function get51Coord(coord) {
 ...
 */
export function ifTwoSegmentsIntersect(line1, line2) {
    const [a, b] = line1;
    const [c, d] = line2;
 ...
 */
export function canCoordsBePolygon(coords) {
    const { length } = coords;
    if (length < 3)
        return false;
 ...
 */
export function setFrameTimeout(callback, frames) {
 ...
 */
export function setFrameInterval(callback, frames) {
    if (!Number.isInteger(frames) || frames <= 0)
 ...
 * 将搜索字符串映射为数据
 */
export function getDataFromQuery(params, fns) {
 ...
 * 将数据映射为搜索字符串
 */
export function setQueryFromData(data, fns, setParams) {
 ...
 * 将对象转换成 innerHTML
 */
export function getStyleInnerHTML(style) {
    return `
${Object.keys(style)
 ...
 * 在 react 中比较数组是否发生变化
 */
export function useArraySignal(data, compareFn) {
 ...
 */
export function drawArc(x, y, radius, startAngle, endAngle, options = {}) {
...
export function createTailwindColors(server) {
    const useTailwindColors = createPersistentStore({ color: "slate", depth: 50 }, "tailwind-colors");
    let socket;
...
export function useAsync(effect, callbackOrDeps, deps) {
